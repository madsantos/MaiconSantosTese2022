...
\chapter{Aplicações HPC sobre Nuvens Computacionais}\label{chap:aplicacoes}
...
\chapter{Nuvens Federadas e a Academia}

A abordagem adotada em diversas situações para solucionar a questão de oferecimento de poder de processamento a instituições de pesquisa é oferecer um suporte de nuvem computacional. As configurações de tais ambientes são diversas, assim como diversas são as estratégias adotadas em sua construção. Não raro, se observa a criação de uma nuvem federada pela simples associação de instituições promovendo o compartilhamento de seus recursos físicos. Estratégias mais elaboradas, envolvendo agências oficiais de fomentos e, eventualmente, políticas de incentivo públicas, também promovem a criação de tais infraestruturas.

Este capítulo apresenta os conceitos fundamentais associados a nuvens federadas, Seção \label{sec:nuvemfederada} e algumas estratégias de escalonamento neste tipo de infraestrutura, na Seção\ref{sec:escalonamentonuvemfederada}, a fim de caracterizar questões relacionadas ao compartilhamento dos recursos. A  Seção \label{sec:iniciativasacademicas} ilustra algumas instâncias desta solução como infraestrutura para o processamento na academia e a Seção \ref{sec:discussao} finaliza este capítulo 

\section{A Nuvem Federada}\label{sec:nuvemfederada}
% \emph{2 páginas}

% Dizer que a ideia é o compartilhamento dos recursos (aquela aposta de que uma instituição nem sempre está utilizando o que possui) e dizer que políticas de escalonamento são importantes para manter taxas de utilização que a tornem interessantes. O texto que começa com \textbf{O paradigma de computação em nuvem federada...} e termina com \textbf{... alta disponibilidade} vem para este ponto.
...
\section{Estratégias de Escalonamento}\label{sec:escalonamentonuvemfederada}
% \emph{2 páginas}
% Aqui é bom ter uma condução para que, depois, a tabela \ref{tab:estrategias} possa ser discutida.
...
\section{Iniciativas Acadêmicas}\label{sec:iniciativasacademicas}
% \emph{2 páginas}

% Aqui aquele texto da proposta. É importante recuperar textos da proposta. Um parágrafo para cada ambiente está ok.

Algumas infraestruturas de nuvem construídas para prover apoio às atividades de pesquisa já são uma realidade. Como exemplo, citam-se a francesa Grid'5000 \cite{balouek_adding_2013}, a americana Chameleon Cloud \cite{keahey_lessons_2020} e a provida pelo projeto australiano Nectar \cite{Nectar2011}. Os projetos tem em comum o provimento de acesso a grandes quantidades de recursos computacionais, com características de reconfiguração e controle destes recursos, permitindo que pesquisadores executem seus experimentos de forma rápida e autônoma, compartilhando dados e resultados.

Estes projetos, geralmente são hospedados em diferentes instituições as quais disponibilizam recursos de hardware para uso compartilhado entre os participantes. Além desta característica de colaboração, as opções de nuvens interinstitucionais se mostram como grandes ambientes de ensaio para experimentos que necessitam ser reproduzíveis, em infraestruturas mais facilmente reconfiguráveis.

O projeto Grid'5000 fornece uma infraestrutura de testes que suporta experimentos em vários tipos de sistemas distribuídos, como computação de alto desempenho, grades, computação em nuvem e outros, em todas as camadas da pilha de software. Do ponto de vista do usuário, o Grid’5000 é um conjunto de sites com exatamente o mesmo ambiente de software. A ideia principal é que os usuários, dispostos a enfrentar a heterogeneidade de software, devem adicionar heterogeneidade controlada durante seus experimentos. Os recursos computacionais são disponibilizados aos usuários de duas maneiras: por meio de uma pesquisa na descrição dos recursos disponíveis e, em seguida, é feita uma reserva, ou então, as necessidades dos usuários são descritas para o sistema, que se encarrega de localizar os recursos apropriados. Uma vez que os recursos solicitados pelo usuário são encontrados, eles podem ser reservados para acesso exclusivo em um determinado momento ou para acesso exclusivo quando estiverem disponíveis.

% As etapas principais identificadas para executar um experimento são (1) encontrar e reservar recursos adequados para o experimento e (2) implantar o aparato do experimento nos recursos. Encontrar recursos adequados pode ser abordado de duas maneiras: ou os usuários pesquisam uma descrição dos recursos disponíveis e, em seguida, fazem uma reserva, ou descrevem suas necessidades para o sistema que localizará os recursos apropriados. Acreditamos que ambas as abordagens devem ser suportadas e, portanto, uma descrição legível por máquina do Grid'5000 está disponível por meio da API de referência. Ele pode ser navegado usando uma interface da web ou executando um programa sobre a API. Ao mesmo tempo, o agendador de recursos em cada site é alimentado com as propriedades do recurso para que um usuário possa solicitar recursos descrevendo as propriedades necessárias (por exemplo, 25 nós conectados ao mesmo switch com pelo menos 8 núcleos e 32 GB de memória) . Uma vez que os recursos correspondentes são encontrados, eles podem ser reservados para acesso exclusivo em um determinado momento ou para acesso exclusivo quando estiverem disponíveis. Neste último caso, um script é fornecido no momento da reserva, como na programação em lote clássica.

O projeto Chameleon Cloud\dots

O projeto Nectar\dots

\section{Discussão}\label{sec:discussao}

A construção e manutenção de uma infraestrutura de nuvem federada requer altos investimentos das instituições envolvidas. Em uma ambiente acadêmico de pesquisa, particularmente considerando o cenário brasileiro, este tipo de investimento, usualmente é parcial, ou mesmo totalmente, coberto por agências de fomento. 
...
\section{Instituições Federadas}\label{sec:instfederadas2}
...
A Tabela \ref{tab:instpar2} apresenta as Instituições Parceiras, que foram relacionadas de acordo com o critério de seleção proposto. Também constam na Tabela \ref{tab:instpar2}, informações sobre qual nível determinada instituição está classificada.
